...
Visual Representations of the top predictors and our target:
...
```{r}
ggplot(train, aes(Alch.Rel, PH)) + geom_point()
```
...
```{r}
ggplot(train, aes(Carb.Rel, PH)) + geom_point()
```

`Temperature` and `PH`

```{r}
ggplot(train, aes(Temperature, PH)) + geom_point()
```
...
The top predictors do not have a clear relationship with `PH`, but based on the visualizations, there are highly concentrated. Given the relationships between the predictors and the target variable are not easily understood, the usage of a complex algorithm like Cubist was necessary. Presenting this information to a decision maker in the beverage company, we can conclude that the most influential contributors are highly concentrated within all brands and mostly produce PH levels above 8, making the beverages alkaline.   
