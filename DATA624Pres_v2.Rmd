...
We're going to create time slices in order to perform cross-validation on a time series. Essentially, training sets consist only of observations that occurr prior to the observation that forms the test set. No future observations can be used in creating the forecast. The approach can be boiled down to creating multiple train/test splits and testing each hyperparameter with multiple training/testing sets. 
...
Here we are creating k-fold cross validation, helping us figure out which configuration is 'best'. **Cross-validation works for time series, if it is autoregressive.** This means that k-fold CV works if the predictors the model are lagged versions of the response variable.
...
* Manuel Tilgner. (2019). **Tuning Random Forests on Time Series Data**. Retrieved from:
